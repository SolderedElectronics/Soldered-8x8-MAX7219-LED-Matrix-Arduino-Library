...
{
    if (_hwDigRows)
        return (copyC(buf, cSrc, cDest));
    else
        return (copyR(buf, cSrc, cDest));
}

bool MD_MAX72XX::copyRow(uint8_t buf, uint8_t rSrc, uint8_t rDest)
{
    if (_hwDigRows)
        return (copyR(buf, rSrc, rDest));
    else
        return (copyC(buf, rSrc, rDest));
}
...
{
    if (_hwDigRows)
        return (setC(buf, c, value));
    else
        return (setR(buf, c, value));
}

bool MD_MAX72XX::setRow(uint8_t buf, uint8_t r, uint8_t value)
{
    if (_hwDigRows)
        return (setR(buf, r, value));
    else
        return (setC(buf, r, value));
}
